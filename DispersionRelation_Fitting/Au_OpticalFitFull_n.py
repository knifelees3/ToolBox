...
def dispersion_n(wavelength, omegaAu_D, gammaAu_D, omegaAu_L, gammaAu_L, omega0_L):
...
if __name__ == '__main__':
    ...
    # print(omegaAu_D1, gammaAu_D1, omegaAu_L1, omegaAu_01, gammaAu_L1)
    para_guess = np.array(
    ...
    num = 1000
    wavelen_mat = np.linspace(500, 2000, num)
    refractive = np.zeros((num, 3))

    filename = "./RefractiveIndexFull.txt"
...
# %%
    fit_para, cov_mat = curve_fit(
        dispersion_n, wl_exp, np.hstack((n_exp, k_exp/30)), para_guess, bounds=(0, np.inf))
# %%
...
# Plot epsilon
    ...
    #plt.xlim([500, 1600])
    plt.grid()
    plt.savefig("./DispersionforAuFull_nk_together.png")
    plt.show()
    ...
    #plt.xlim([500, 1600])
    plt.grid()
    plt.savefig("./DispersionforAuFull_n.png")
    plt.show()
